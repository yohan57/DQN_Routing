...
class Environment(object):
    ...
    def get_valid_actions(self, cell=None):
        ...
        if row>0:
            if self.grid[row-1,col] == obstacle_cell_mark or self.grid[row-1,col] == visited_cell_mark:
                actions.remove(UP)
        if row<nrows-1:
            if self.grid[row+1,col] == obstacle_cell_mark or self.grid[row+1,col] == visited_cell_mark:
                actions.remove(DOWN)
        ...
        if col>0:
            if self.grid[row,col-1] == obstacle_cell_mark or self.grid[row,col-1] == visited_cell_mark:
                actions.remove(LEFT)
        if col<ncols-1:
            if self.grid[row,col+1] == obstacle_cell_mark or self.grid[row,col+1] == visited_cell_mark:
                actions.remove(RIGHT)
    ...
    def update_obstacle_cells(self, new_obstacle_cells):
        for cell in new_obstacle_cells:
            self.grid[cell[0], cell[1]] = obstacle_cell_mark
...
count = 0
for start, dest in start_dest_pair:
    temp_env = Environment(routing2.shape, start, dest, path_list)
    model = build_model(temp_env)
    qtrain(model, routing2.shape, start, dest, path_list, epochs=1000, max_memory=8*temp_env.grid.size, data_size=32, name="result_weight")
    demonstrate(model, count, path_list, routing2.shape, start, dest)
    count += 1